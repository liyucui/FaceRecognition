...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    #RNet use 3 tfrecords to get data
        ...
        pos_radio = 1.2/6;part_radio = 1.2/6;neg_radio=3.6/6
        ...
        neg_batch_size = int(np.ceil(config.BATCH_SIZE*neg_radio))-1
    ...
    if net == 'PNet':
        image_size = 12
        radio_cls_loss = 1.0;radio_bbox_loss = 0.8; # radio_landmark_loss = 0.5;
    elif net == 'RNet':
        image_size = 24
        radio_cls_loss = 1.0;radio_bbox_loss = 0.8; #radio_landmark_loss = 0.5;
    else:
        radio_cls_loss = 1.0;radio_bbox_loss = 0.8; #radio_landmark_loss = 1;
        image_size = 48
    ...
    cls_loss_op,bbox_loss_op,L2_loss_op,accuracy_op = net_factory(input_image, label, bbox_target,landmark_target=None,training=True)
    ...
    try:
        ...
        for step in range(MAX_STEP):
            ...
            if (step+1) % display == 0:
                ...
                # landmark loss: %4f,
                print("%s : Step: %d/%d, accuracy: %3f, cls loss: %4f, bbox loss: %4f,L2 loss: %4f, Total Loss: %4f ,lr:%f " % (datetime.now(), step+1, MAX_STEP, acc, cls_loss, bbox_loss, L2_loss,total_loss, lr))
                # 关键点
