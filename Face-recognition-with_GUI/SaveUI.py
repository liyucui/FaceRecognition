...
class MainUi(Initor_for_event):
    ...
    def __init__(self):
        super().__init__()
        self.pic_dir='F:/GitHub项目/Face_recong_test/禾苗persons' #Save register raw img
        self.init_ui()
    ...
    def getText(self):
        ...
        if ok is True:
            save_pt = os.path.join(self.pic_dir, text).replace('\\', '/')
            self.detect_thread.start()
            if not os.path.exists(os.path.join(save_pt)):
                os.mkdir(save_pt)
            if img2save is None:
                QMessageBox.about(self, '消息', '{} 添加失败。'.format(text))
            else:
                cv2.imencode('.png', img2save)[1].tofile(os.path.join(
                    save_pt, '{}.jpg'.format(text)))
    ...
    def load_camera_video(self, index=1):

        self.cap = cv2.VideoCapture(1)
        self.cap.set(6, cv2.VideoWriter.fourcc('M', 'J', 'P', 'G'))
        self.cap.set(3, 1920)
        self.cap.set(4, 1080)
        # 调用摄像头（一般电脑自带摄像头index为0）
